...
static void tps80031_irq_enable(unsigned int irq)
{
	...
	if (data->is_sec_int) {
		tps80031->cont_int_mask_reg &= ~(1 << data->int_mask_bit);
		tps80031->cont_int_en |= (1 << data->int_mask_bit);
		tps80031->mask_reg[data->mask_reg] &= ~(1 << data->mask_mask);
		tps80031->irq_en |= (1 << data->parent_int);
	} else
		tps80031->mask_reg[data->mask_reg] &= ~(1 << data->mask_mask);
...
static void tps80031_irq_disable(unsigned int irq)
{
	...
	if (data->is_sec_int) {
		...
		if (!tps80031->cont_int_en) {
			tps80031->mask_reg[data->mask_reg] |=
						(1 << data->mask_mask);
			tps80031->irq_en &= ~(1 << data->parent_int);
		}
		tps80031->irq_en &= ~(1 << __irq);
