...
static struct tegra_sdhci_platform_data tegra_sdhci_platform_data3 = {
	...
	.cd_gpio = -1,
	.wp_gpio = -1,
	.power_gpio = SMBA1007_SDINT_POWER,
...
{
	pr_debug("%s: %d\n", __func__, on);
	gpio_set_value(SMBA1007_WL_BT_POWER, on);
	mdelay(100);
...
static int __init smba1007_wifi_prepower(void)
{
	if (!machine_is_smba1007())
		return 0;

	smba1007_wifi_power(1);

	return 0;
}

subsys_initcall_sync(smba1007_wifi_prepower);
#endif
...
static int __init smba_wifi_init(void)
{
	...
	gpio_request(SMBA1007_WL_BT_POWER, "wlan_power");
	gpio_request(SMBA1007_WLAN_RESET, "wlan_rst");

	gpio_direction_output(SMBA1007_WL_BT_POWER, 0);
