/*
 * arch/arm/mach-tegra/board-smba1007-panel.c
 *
...
static int smba_backlight_init(struct device *dev)
{
	...
	ret = gpio_request(SMBA1007_BL_ENB, "backlight_enb");
	if (ret < 0)
		return ret;

	ret = gpio_direction_output(SMBA1007_BL_ENB, 1);
	if (ret < 0)
		gpio_free(SMBA1007_BL_ENB);
...
{
	gpio_set_value(SMBA1007_BL_ENB, 0);
	gpio_free(SMBA1007_BL_ENB);
}

static int smba_backlight_notify(struct device *unused, int brightness)
{
	gpio_set_value(SMBA1007_EN_VDD_PANEL, !!brightness);
	gpio_set_value(SMBA1007_LVDS_SHUTDOWN, !!brightness);
	gpio_set_value(SMBA1007_BL_ENB, !!brightness);
	return brightness;
}
...
{
	gpio_set_value(SMBA1007_EN_VDD_PANEL, 1);
	mdelay(smba_pnl_to_lvds_ms);
	gpio_set_value(SMBA1007_LVDS_SHUTDOWN, 1);
...
{
	gpio_set_value(SMBA1007_LVDS_SHUTDOWN, 0);
	gpio_set_value(SMBA1007_EN_VDD_PANEL, 0);
	return 0;
}
...
static struct tegra_dc_out smba_disp2_out = {
	...
	.dcc_bus	= 1,
	.hotplug_gpio	= SMBA1007_HDMI_HPD,
...
int __init smba_panel_init(void)
{
	...
	gpio_request(SMBA1007_EN_VDD_PANEL, "pnl_pwr_enb");
	gpio_direction_output(SMBA1007_EN_VDD_PANEL, 1);

	gpio_request(SMBA1007_BL_VDD, "bl_vdd");
	gpio_direction_output(SMBA1007_BL_VDD, 1);

	gpio_request(SMBA1007_LVDS_SHUTDOWN, "lvds_shdn");
	gpio_direction_output(SMBA1007_LVDS_SHUTDOWN, 1);

	gpio_request(SMBA1007_HDMI_HPD, "hdmi_hpd");
	gpio_direction_input(SMBA1007_HDMI_HPD);
